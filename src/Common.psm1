...
function FindInspectCode
{
    ...
    $inspectcode = Join-Path $toolsDir "JetBrains.ReSharper.CommandLineTools.2020.1.4\tools\inspectcode.exe"

    if (!(Test-Path $inspectcode))
    {
        throw "The inspectcode.exe could not be found at: $inspectcode;" + `
               "did you install it with nuget " + `
               "(see $( Join-Path $PSScriptRoot "InstallToolsForBuildTestInspect.ps1" ))?"
    }
    return $inspectcode
}
...
{
    $toolsDir = GetToolsDir
    $reportGenerator = Join-Path $toolsDir "ReportGenerator.4.6.1\tools\net47\ReportGenerator.exe"
